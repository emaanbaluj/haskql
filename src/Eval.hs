  ...
  case query of
    ...
    Merge mergeType var1 var2 colNum -> do
      ...
      where
        ...
        mergeRows LeftMerge leftRowPairs = map (map (\(p, q) -> if null p then q else p)) leftRowPairs
        mergeRows RightMerge rightRowPairs = map (map (\(p, q) -> if null q then p else q)) rightRowPairs
